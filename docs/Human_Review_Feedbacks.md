...
## Review 6：第四层/第五层——流水线与 Worker（2026-05-08）

审查范围：`nexus_app/worker/claimer.py`、`nexus_app/worker/runner.py`、`docs/企业数据与知识资产平台nexus_v7.1.md`。

---

### FB-6-1：Job.priority 排序方向需统一并文档化

**原始意见**

> Claim 排队中 job 时，优先级排序 `models.Job.priority.asc()` 是否正确，priority 值越大优先级越高或者值越小越高，无论哪种，我们应该统一策略，且文档中明确说明。

**Action: 已实施**

- 统一策略：`priority` 数值越小优先级越高。
- `worker/claimer.py` 保持 `ORDER BY priority ASC, created_at ASC`（SQLite 和 PostgreSQL 两个分支一致）。
- `models.py` 的 `Job.priority` 字段新增注释：`Lower value = higher priority (e.g., 10 > 100 > 200)`。
- `v7.1` 文档 §3.3.1 新增优先级说明条目，明确 Worker claim 顺序。

**理由**

小值高优先级是作业调度常见约定（0/10 代表紧急），且已在实现中落地。关键在于统一语义并文档化，避免后续调用方误把大值当高优先级写入。

---

### FB-6-2：v7.1 文档中 Pipeline A/B 链路描述需核查

**原始意见**

> 流水线执行顺序 `assetize → parse（documents only）→ normalize` 没问题，但 v7.1 文档 3.3.3 两条处理管道描述似乎不正确，请核查。

**Action: 已实施**

- 核查结论：文档中存在顺序偏差，`assetize` 被放在两管道汇合后，和代码实际执行顺序不一致。
- 修正 `v7.1` 文档：
  - §3.3.3 管道表格补充 `assetize` 阶段（A/B 两管道都在 parse/normalize 之前执行）。
  - §3.3.3 分步链路（管道 A/B）改为：接入 → Job 入队 → assetize → parse/normalize 分叉。
  - §3.1 逻辑层图中的两条管道描述同步补充 `assetize`。

**理由**

`assetize` 是后续 `parse_artifact` / `normalized_asset_ref` 的归属锚点（需要先有 `asset_version.id`），在链路顺序上不能后置。文档必须与代码一致，否则会误导后续治理和索引阶段设计。

---

### FB-6-3：record 管道 raw_payload 非真时应快速失败

**原始意见**

> execute_job 主流程可接受，后续会扩展 AI 治理等步骤；但需要审核健壮性，raw_payload 非真时应该快速失败并记录问题。

**Action: 已实施**

- `worker/runner.py` 中 record 管道读取 raw JSON 逻辑改为**快速失败**：
  - JSON 解析异常 → 立即标记 job `FAILED`，`last_error_code=invalid_record_payload`，写 `PIPELINE_FAILED` 审计，并抛 `NonRetryableError`。
  - JSON 解析成功但 payload 不是非空 object（`{}` / 非 dict）→ 同样快速失败并记录。
- 不再使用旧逻辑的 `except: raw_payload = {}` 继续执行。
- 新增测试：`test_record_pipeline_fails_fast_on_invalid_raw_payload`（BadJsonStorage 注入非法 JSON，断言 job 失败和错误码正确）。

**理由**

record 管道的标准化依赖原始 JSON 语义。若 payload 无效仍继续执行，会产生无意义资产版本并污染后续治理链路。快速失败可将异常限制在接入后最早阶段，降低错误扩散面，也便于运维定位。

---

*最后更新：2026-05-08（Review 6 完成后）*


## Review 7：wk1 + wk2 深度查缺补漏（2026-05-13）

审查范围：wk1 / wk2 任务包交付物全量对照，覆盖 `enums.py`、`services.py`、`pipeline/stages.py`、`ingest/gateway.py`、`nexus-api/api/v1.py`。

---

### FB-7-1：审计事件覆盖率不足（追溯率 < 100%）

**原始意见**

> wk1 TP-W1-01 要求"审计事件初版清单"，当前 `enums.py` 只有 5 个事件，缺少资产版本归档、数据源创建、API Key 创建/吊销等生命周期事件。`stages.py` 归档旧版本时无审计写入。

**Action: 已实施**

- `enums.py` 新增 5 个 `AuditEventType` 值：
  - `ASSET_VERSION_ARCHIVED`：资产版本被新版本取代时归档
  - `DATA_SOURCE_CREATED`：数据源注册
  - `DATA_SOURCE_STATUS_CHANGED`：数据源状态变更（预留）
  - `API_CALLER_CREATED`：API Key 创建
  - `API_CALLER_REVOKED`：API Key 吊销（预留）
- `pipeline/stages.py` `run_assetize` 归档旧版本循环内补写 `ASSET_VERSION_ARCHIVED` 审计事件，记录 `asset_id`、`version_no`、`reason=superseded_by_new_ingest`。
- `services.py` `create_data_source` 和 `create_api_caller` 新增 `trace_id`、`actor_type`、`actor_id` 可选参数，并在 flush 后写入对应审计事件。

---

### FB-7-2：multipart 端点 base64 往返转换

**原始意见**

> `submit_ingest_file_upload` 读取文件 bytes 后 encode 为 base64，再由 `FileUploadAdapter.prepare()` decode，对大文件造成不必要内存压力（实际内存占用是文件大小的 2 倍以上）。

**Action: 已实施**

- `ingest/gateway.py` 新增 `submit_file_bytes()` 函数，直接接受 `bytes`，构造 `PreparedContent` 后调用 `_submit_ingest`，完全绕过 base64 编解码。
- `nexus-api/api/v1.py` 的 `submit_ingest_file_upload` 改为调用 `submit_file_bytes`，不再经过 `IngestFileSubmit.content_base64`。

---

### FB-7-3：DataSource / ApiCaller CRUD 无审计

**原始意见**

> CLAUDE.md 明确要求"Any code that changes governance, permission, Prompt, rule, version status, API key, or AI adoption state must write audit logs"。`create_data_source` 和 `create_api_caller` 均未写审计。

**Action: 已实施**（见 FB-7-1，合并处理）

---

### FB-7-4：connection_config 无输入校验

**原始意见**

> `DataSource.connection_config` 字段已加，对应 Pydantic 模型也已加，但 `create_data_source` 不做任何校验就直接存库。

**Action: 已实施**

- `nexus-api/api/v1.py` 新增 `_validate_connection_config(source_type, config)` 函数，按 `source_type` 选择对应 Pydantic 模型（`NasConnectionConfig`、`CrawlerConnectionConfig`、`DatabaseConnectionConfig`、`WebhookConnectionConfig`）做 `model_validate`。
- `create_data_source` 路由在调用 `services.create_data_source` 前执行校验，不通过返回 422。
- `file_upload` 类型无 `connection_config` 约束，校验函数对未知 `source_type` 直接放行。
...
> `TP-W2-03` 中作业状态 `pending` 已改为 `queued`，作业类型 `parse/normalize/assetize` 已被 Review 4 删除，任务包未同步更新。

**Action: 已实施**

- `wk_2_task_package.md` TP-W2-03 Scope 中作业类型和状态枚举更新为实际实现值，并注明变更来源（Review 4 FB-4-3）。

---

*最后更新：2026-05-13（Review 7 完成后）*
