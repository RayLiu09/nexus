---
title: 企业数据与知识资产平台技术选型和架构nexus_v2.4
created: '2026-05-06'
modified: '2026-05-06'
---

# 企业数据与知识资产平台技术选型和架构 v2.4 — NEXUS
...
| v2.4 | 2026-05-06 | 补齐 PostgreSQL 作业表 + 后台 Worker 轮询的异步作业设计细节，明确单节点 P0 并发处理边界和升级触发条件；明确导入数据源默认分级仅采用 L1/L2，L3/L4 仅作为显式例外分级。 |

---

## 一、文档目的

本文档基于 v2.3 架构基线和对应 Review 意见，输出适配**数据资产规模较小**场景的 v2.4 技术架构基线。

v2.4 的核心调整方向是：在保留 P0 完整业务能力和 v2.3 精简结论的前提下，将 PostgreSQL 作业表 + 后台 Worker 轮询从“选型结论”补充为可实现、可运维、可验收的设计；同时统一导入数据源的默认分级口径，避免 P0 阶段因为默认 L3/L4 带来不必要的权限、脱敏和人工复核复杂度。

v2.4 继续回答 v2.3 所有架构问题（系统边界、身份组织、主数据约束、版本状态、AI 治理、规则配置、基础设施精简等），同时新增回答：

12. PostgreSQL 作业表 + Worker 轮询如何实现作业认领、锁租约、重试、超时恢复、并发控制和审计追踪。
13. 单节点 P0 部署可以支撑的推荐并发处理量是多少，超过什么条件必须升级到 MQ / 多节点。
14. 导入数据源的默认分级如何控制在 L1/L2，L3/L4 如何作为显式例外进入治理流程。

---

## 二、本轮 Review 结论与 v2.4 修正

### 2.1 Review 发现

| 序号 | Review 意见 | 既有问题 | v2.4 修正 |
|------|-------------|-----------|-----------|
...
| R-07 | PostgreSQL 作业表方案需要可执行设计 | v2.3 仅说明使用 PostgreSQL 作业队列，未定义认领 SQL、行锁语义、租约超时、心跳、重试、索引和 Worker 并发上限 | 补充 `job` 表字段、`FOR UPDATE SKIP LOCKED` 认领模式、租约与心跳、指数退避、死信状态、调度索引、幂等键和 Worker 并发配置；明确单节点推荐 8-12 个活跃流水线作业、上限 16 个活跃作业，文档解析重作业建议 2-4 并发 |
| R-08 | 导入数据源默认分级需要收敛 | 若导入时默认允许 L3/L4，P0 将被迫实现更多高敏脱敏和复核流程，且与当前业务样本不匹配 | 明确除非数据源配置、规则命中或人工/安全审批显式声明，所有导入数据源默认初始分级只能是 L1/L2；L3/L4 为例外分级，必须进入审计和必要复核 |
...
8. PostgreSQL 作业队列是 P0 默认实现，必须使用行级锁、短事务认领、租约超时和心跳恢复避免重复执行；Worker 并发由配置控制，不能无限制按队列表行数扩张。
9. 单节点 P0 容量以稳定性为优先：推荐同时运行 8-12 个活跃流水线作业，单机硬上限 16 个活跃作业；其中 MinerU 文档解析重作业建议 2-4 并发，标准化/治理/索引同步等轻作业建议 4-8 并发。
10. 导入数据源默认分级仅采用 L1/L2；L3/L4 只能由显式数据源配置、敏感识别规则、人工复核或安全例外提升，且必须写入审计。
11. 架构升级路径清晰：当数据规模增长、规则频繁变更或需要合规审计时，可逐项拆分 `governance_result` 内嵌 JSONB、恢复 `quality_report` 和 `governance_decision_log` 实体、升级规则生命周期、引入 Redis 和 MQ，每项独立可演进。
...
15. **导入分级默认低敏。** 除非数据源配置、敏感识别规则、人工复核或安全例外显式声明，所有导入数据源的初始分级只能是 L1/L2；L3/L4 不作为默认值，只作为被证据触发的例外治理结果。
...
分级可见性检查：导入数据源默认初始分级仅采用 L1/L2；L1/L2 按 RBAC 角色和组织范围放行。访问 L3/L4 资产时需显式角色授权，返回时默认屏蔽已标记敏感字段。

默认分级规则：

1. `data_source.default_level_hint` 为空时，系统默认按 L2 处理；明确公开资料、公开政策、公开标准等可配置为 L1。
2. 数据源接入配置不得默认写入 L3/L4，除非安全负责人、平台/数据管理员或业务规则已明确标记该来源包含高敏数据。
3. AI 或规则可基于敏感字段、个人信息、内部经营数据、跨组织范围等证据将版本级治理结果提升到 L3/L4；提升原因必须写入 `governance_result.decision_trail` 和 `audit_log`。
4. L3/L4 提升不得只依赖文件名或来源路径，必须结合标准化内容、字段或人工确认。
...
| 字段级脱敏 | L3/L4 数据通过显式例外进入平台，且需要 API 输出时自动脱敏敏感字段 |
...
说明：v2.4 P0 作业编排以 PostgreSQL 作业队列 + 后台 Worker 轮询为默认实现，不要求部署 RabbitMQ；Redis 为可选组件；AI 大模型能力依赖既有 LiteLLM；NEXUS 不额外开发 AI 网关。
...
| `source_type` | enum | 必填 | `file_upload` / `nas` / `crawler` / `database` / `webhook`。 |
| `default_domain` | enum | 可空 | D1-D6 默认数据域提示，不是最终治理结论。 |
| `default_level_hint` | enum | 可空 | 默认分级提示，仅允许为空、L1、L2；为空时按 L2 处理。 |
| `default_org_scope` | string / json | 可空 | 默认组织范围提示。 |
| `allow_high_sensitivity_override` | bool | 默认 false | 是否允许该来源通过规则或人工显式提升到 L3/L4。 |
| `high_sensitivity_reason` | text | 可空 | 允许 L3/L4 例外时的审批或业务理由。 |
| `status` | enum | 必填 | `enabled` / `disabled` / `error`。 |

约束：

1. 除非显式审批或规则配置声明，所有导入数据源只能设置为空、L1 或 L2 的默认分级提示。
2. L3/L4 不是数据源默认值；当确需导入高敏数据时，必须设置 `allow_high_sensitivity_override = true` 并记录理由、审批主体和审计事件。
3. 数据源默认分级只是治理提示，不产生最终分级；最终分级仍由标准化对象、AI 建议、敏感识别、规则护栏和人工复核共同决定。
4. 对既有未标注数据源，迁移或初始化时默认按 L2 处理。
...
### 7.6 治理结果字段约束（v2.4 enriched）

#### `governance_result`

v2.4 延续 v2.3 的精简结论：`governance_result` 承担 v2.2 中 `quality_report` 和 `governance_decision_log` 的核心功能，通过两个 JSONB 字段内嵌，减少实体数量。
...
4. L3/L4 例外分级、高敏、Schema 校验失败不得由 AI 直接自动发布，必须经过规则护栏或人工复核。
...
document_version
    ...
    │       └── 证据引用、置信度
    │
    └── governance_result（v2.4 enriched）
...
processing
    ...
    │    available
    │
    ├── 治理字段缺失 / AI 低置信度 / AI 与规则冲突 / L3/L4 例外风险 / 组织范围不明 / 索引准入失败
    │       ▼
    │    review_required
    │
    └── 解析失败 / 标准化失败 / 不可恢复错误
...
| 分级 | 默认导入分级为 L1/L2；基于敏感字段、内容风险、组织范围生成 L1-L4 建议 | 高敏优先，L3/L4 不允许无护栏自动发布；默认不得从数据源直接写入 L3/L4 | L3/L4 例外、敏感冲突、跨组织风险或低置信度 |
...
search-service 重排、知识组织、NEXUS 二次权限校验、来源引用回写
    ▼
nexus-api 返回结果（L3/L4 例外数据按需脱敏）+ 审计
```
...
v2.4 将异步作业编排分为 P0 默认和扩展两个层次：

| 层次 | 选型 | 适用场景 | 升级触发条件 |
|------|------|---------|-------------|
| **P0 默认** | PostgreSQL 作业表 + 后台 Worker 进程轮询 | 单机或小规模部署，作业并发量低，无需跨节点路由 | 单节点活跃作业长期超过 16、队列等待 P95 超过 5 分钟、或需要多优先级路由和独立死信监控 |
| 扩展选型 | RabbitMQ + Celery | 多节点部署、高并发解析任务、需要精细路由和死信处理 | 按上述触发条件引入 |

#### 12.3.1 PostgreSQL 作业表模型

`job` 是 P0 作业队列和状态中心的唯一事实来源。建议字段：

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | UUID | 作业 ID。 |
| `job_type` | enum | `ingest_process` / `parse` / `normalize` / `assetize` / `ai_governance` / `rule_governance` / `rag_sync` / `reprocess` / `re_governance`。 |
| `status` | enum | `queued` / `running` / `succeeded` / `failed` / `review_required` / `dead_lettered` / `cancelled`。 |
| `priority` | int | 数值越小优先级越高；P0 默认 `100`，不做复杂路由。 |
| `next_run_at` | timestamptz | 最早可执行时间；重试用指数退避更新。 |
| `locked_by` | string | 当前认领 Worker ID。 |
| `locked_at` | timestamptz | 当前认领时间。 |
| `lock_expires_at` | timestamptz | 租约过期时间；Worker 崩溃后由其他 Worker 接管。 |
| `heartbeat_at` | timestamptz | Worker 执行中定期心跳。 |
| `attempt_count` | int | 已尝试次数。 |
| `max_attempts` | int | 最大尝试次数；默认 3，解析类重作业可配置为 2。 |
| `idempotency_key` | string | 作业幂等键，防止重复创建有效作业。 |
| `payload` | jsonb | 作业参数，只保存对象 ID、版本 ID、URI、配置版本等引用，不保存大段正文。 |
| `last_error_code` | string | 最后错误码。 |
...
1. `idx_job_polling(status, next_run_at, priority, created_at)`：Worker 查询待执行作业。
2. `idx_job_lock_expiry(status, lock_expires_at)`：扫描超时运行作业。
3. `idx_job_idempotency(job_type, idempotency_key)`：幂等创建。
4. `idx_job_targets(asset_id, version_id, raw_object_id, ingest_batch_id)`：作业中心和回溯查询。

#### 12.3.2 Worker 轮询与认领语义

Worker 不在长事务中执行耗时任务。认领事务只完成“选中 + 加锁 + 标记 running”，随后提交事务并在事务外执行任务。
...
SET status = 'running',
    locked_by = :worker_id,
    locked_at = now(),
    lock_expires_at = now() + (:lease_seconds * interval '1 second'),
    heartbeat_at = now(),
...
1. `FOR UPDATE SKIP LOCKED` 防止多个 Worker 重复认领同一作业。
2. `batch_size` 不应超过 Worker 剩余并发槽位；P0 默认 1-4。
3. Worker 执行中每 30-60 秒更新 `heartbeat_at` 和 `lock_expires_at`。
4. Worker 正常完成时写入 `succeeded`、`review_required` 或业务定义的终态，并释放锁字段。
5. Worker 失败时根据错误类型决定重试或终态失败，错误摘要必须脱敏。
6. API 进程可以创建作业，但不得直接执行耗时解析、治理或索引同步。

#### 12.3.3 租约超时、重试与死信

租约恢复：

1. 扫描 `status = running AND lock_expires_at < now()` 的作业。
2. 若 `attempt_count < max_attempts`，将状态改回 `queued`，设置新的 `next_run_at`。
3. 若超过最大尝试次数，改为 `dead_lettered` 或 `failed`，并写入 `JobDeadLettered` / `PipelineFailed` 审计事件。
...
| 第 2 次失败 | 5 分钟 |
| 第 3 次失败 | 15 分钟 |
| 超过 `max_attempts` | `dead_lettered` 或 `failed` |

错误分类：

- 可重试：MinerU 短时不可用、MinIO 临时网络错误、LiteLLM 超时、RAGFlow 临时失败、数据库死锁。
- 不可重试：输入文件格式不支持、校验摘要不一致、标准化 Schema 不可恢复、规则表达式非法、权限策略阻断。
- 需复核：低置信度、高敏例外、组织范围不清、AI/规则冲突、质量准入失败。

#### 12.3.4 单节点并发处理边界

在 16 Core / 64 GB RAM / 1 张 48 GB GPU 的单节点 P0 基线下，NEXUS 以稳定处理为目标，不以最大吞吐为目标。

| 并发项 | 推荐值 | 单节点上限 | 说明 |
|--------|--------|------------|------|
| 活跃流水线作业总数 | 8-12 | 16 | 包含解析、标准化、治理、索引同步等所有 `running` 作业。 |
| MinerU 文档解析作业 | 2-4 | 4 | 解析是 CPU/GPU/IO 重作业，优先限制并发。 |
| 标准化作业 | 4-8 | 8 | 主要消耗 CPU/内存和对象存储 IO。 |
| AI 治理 / 质量评分作业 | 2-4 | 6 | 受 LiteLLM 模型并发、上下文长度和限流影响。 |
| 规则治理作业 | 4-8 | 12 | 通常较轻，但应受数据库连接池限制。 |
| RAGFlow 同步作业 | 2-4 | 6 | 受 RAGFlow 索引能力和向量化吞吐影响。 |
| API 数据库连接池 | 10-20 | 30 | API 和 Worker 必须共享数据库容量预算。 |
| Worker 数据库连接池 | 8-16 | 24 | 不得按作业数量无限扩展连接。 |

容量口径：

1. 推荐同时处理 8-12 个活跃流水线作业，保障 API、控制台和作业中心仍可响应。
2. 单节点超过 16 个活跃作业不再作为 P0 保证范围；可短时排队，但不应长期运行。
3. 小批量导入建议单批 10-50 个文件或记录包；更大批次应拆成多个批次并由队列慢慢消化。
4. 单节点目标是“小时级消化小批量、分钟级定位失败”，不是高吞吐批处理平台。
5. 真实吞吐受文件页数、MinerU 模式、模型上下文、RAGFlow 索引速度影响；上线前必须用业务样本压测校准。

升级触发条件：

1. `queued` 作业等待时间 P95 连续 3 天超过 5 分钟。
2. `running` 作业长期超过 16，或解析作业长期达到 4 并发仍无法按业务窗口消化。
3. PostgreSQL CPU 超过 70% 或锁等待明显影响 API P95。
4. 需要按作业类型做独立队列、优先级路由、暂停/恢复队列或独立死信监控。
5. 需要多台 Worker 横向扩展并跨节点隔离解析、治理、索引资源。

触发后引入 RabbitMQ + Celery。PostgreSQL `job` 表仍保留为状态中心和审计事实来源，MQ 只承担投递、路由和并发调度。
...
3. **资产授权**：组织范围决定资产可见性；导入默认 L1/L2，分级提升到 L3/L4 时需要显式角色授权。
4. **检索过滤**：`search-service` 将组织范围编译为 RAGFlow metadata filter；返回前执行 NEXUS 侧二次组织范围校验。
5. **输出控制**：L3/L4 例外数据访问写入审计；已标记敏感字段在返回时屏蔽（P0 按实际 L3/L4 数据存在情况实现）。
...
以下能力在 v2.4 中仅做架构预留，不做一期具体设计与实现：
...
以下能力在 v2.4 延续 v2.3 的精简结论，并补充作业队列与默认分级升级边界。每项均有明确升级触发条件：
...
| RabbitMQ + Celery | 可选扩展 | 单节点活跃作业长期超过 16、队列等待 P95 连续 3 天超过 5 分钟、或需要多优先级路由 / 独立死信监控 | 引入 RabbitMQ，将 Worker 改为 Celery Worker；PostgreSQL 作业表保留为状态持久化和审计事实来源 |
...
| 字段级脱敏 | L3/L4 例外数据存在时实现 | L3/L4 数据通过显式例外进入平台，且 API 需要按字段脱敏输出 | `nexus-api` 在 `search-service` 返回后增加字段脱敏层 |
| 默认分级 L1/L2 | 导入数据源默认低敏 | 业务确认某数据源长期包含 L3/L4，且具备审批、脱敏、复核和审计机制 | 对该数据源开启 `allow_high_sensitivity_override`，并通过规则或人工显式提升分级 |
...
| 高可用升级 | v2.4 明确边界 | 拆分数据库高可用、检索节点扩展、对象存储多副本。 |
...
| 权限过滤 | 未授权组织范围的资产不得进入检索结果；导入默认 L1/L2，L3/L4 例外字段在有实际高敏数据时脱敏。 |
