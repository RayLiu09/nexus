...
### 7.1 model_version 选择规则

| 文件 mime_type | model_version | 说明 |
|---------------|--------------|------|
| `text/html`、`application/xhtml+xml` | `MinerU-HTML` | 必须显式指定 |
| 其他文档格式（默认） | `pipeline` | 成本低、速度快，适合大批量处理 |
| 复杂排版 / 图文混排（可选升级） | `vlm` | 多模态能力更强，GPU 成本更高 |

调用方可通过 `Job.payload.model_version_override` 强制指定，覆盖自动路由结果。

### 7.2 OCR 自动开启规则

| 触发条件 | OCR 设置 |
|---------|---------|
| mime_type 包含 `image/` | `ocr_enable=true` |
| mime_type 为 `application/pdf` | `ocr_enable=true` |
| mime_type 包含 `tiff` | `ocr_enable=true` |
| 其他文档格式 | `ocr_enable=false` |
