...
1. Static D4 PDF ingestion produces asset, version, parse artifact, normalized ref, AI run, quality report, governance result, chunks, and index manifest.
...
5. Rule publish and re-governance generate new decision logs and mark index stale if needed.
6. Unauthorized caller cannot retrieve L3/L4 content.
...
12. AI re-score produces new `ai_governance_run` and `quality_report` while retaining feedback and score deltas.

## Permission Cases

1. Cross-org access denied by default.
2. L4 content masked by default.
...
1. Prompt create, validate, publish, and disable are audited.
2. Rule create, validate, publish, and rollback are audited.
