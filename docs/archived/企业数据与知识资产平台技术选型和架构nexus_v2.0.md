---
title: 企业数据与知识资产平台技术选型和架构nexus_v2.0
created: '2026-04-27'
modified: '2026-04-27'
---

# 企业数据与知识资产平台技术选型和架构 v2.0 — NEXUS
...
本文档基于 [企业数据与知识资产平台nexus_v7.0.md](/home/bjbodao/projects/nexus/docs/企业数据与知识资产平台nexus_v7.0.md)、[企业数据与知识资产平台需求Spec_v1.2.md](/home/bjbodao/projects/nexus/docs/企业数据与知识资产平台需求Spec_v1.2.md)、[企业数据与知识资产平台技术选型和架构nexus_v1.3.md](/home/bjbodao/projects/nexus/docs/企业数据与知识资产平台技术选型和架构nexus_v1.3.md) 和本轮架构 Review 意见，输出 NEXUS 一期工程落地的 v2.0 技术架构基线。
...
## 二、本轮 Review 结论与 v2.0 修正

### 2.1 Review 发现

| 序号 | Review 意见 | v1.3 问题 | v2.0 修正 |
|------|-------------|-----------|-----------|
...
8. 身份组织本地可控。NEXUS 自维护用户和组织主数据，钉钉同步仅作为可选数据来源。
9. 一期聚焦业务主链路。发布、监控、告警、容量规划等运维业务只做架构预留，不做具体产品化设计和实现。
10. 技术选型以私有化、可替换、可扩展为前提，不将平台生命周期绑定到单一外部系统。
...
normalized_document / normalized_record
    │
    ├── metadata-enrich（候选生成、质量评分、敏感识别）
    │
    └── governance-rule（规则配置、规则执行、冲突处理、决策追踪）
    │
    ▼
...
说明：v2.0 不设置独立运维业务模块。发布、监控、告警、容量规划仅在接口、日志、健康检查和部署规范上预留扩展点。
...
| `metadata-enrich` | 必做 | 基于标准化对象生成分类、分级、标签、质量评分和敏感识别候选 | 治理候选、`quality_report` |
...
为避免在主表保存可推导字段，v2.0 引入只读视图或等价查询封装：
...
| `check_items` | json | 必填 | 正文完整性、结构完整性、字段完整性、切片准备度等检查项。 |
...
| `decision_source` | enum | 必填 | `auto_rule` / `manual_review` / `system_default`。 |
...
| `decision_status` | enum | 必填 | `auto_passed` / `review_required` / `manual_overridden` / `failed`。 |
...
7. `metadata-service` 不直接从 `raw_object` 或 MinerU 原始输出生成最终分类、分级、标签和组织范围；它必须消费标准化对象或标准化对象摘要。
8. `document_asset` 可保存从当前可用版本继承而来的资产级分类、分级、标签、组织范围基线，但版本级治理结果仍以 `governance_result` 为准。
9. RAGFlow 索引投影必须由当前标准化引用、正式治理结果和权限范围共同生成。
...
processing
    │
    ├── 标准化成功 + 当前有效标准化引用 + 质量达标 + 治理规则自动通过 + 索引准入允许
    │       ▼
    │    available
    │
    ├── 治理字段缺失 / 规则冲突 / 低置信度 / L4 风险 / 组织范围不明 / 索引准入失败
    │       ▼
    │    review_required
    │
    └── 解析失败 / 标准化失败 / 不可恢复错误
...
| 质量报告通过 | `quality_report` 达到质量准入阈值，且无不可恢复结构缺陷。 |
| 治理结果有效 | 存在 `governance_result.status = effective`，且分类、分级、标签、组织范围满足必填规则。 |
...
4. 接入登记中的来源信息和默认治理提示
5. `quality_report` 中的结构完整性、正文可用性、字段质量和切片准备度
6. 敏感字段识别、关键词识别和实体识别候选
7. `identity-org-service` 中的组织范围和用户上下文
...
| 元数据增强 | 标准化对象 + 来源提示 + 组织上下文 | 分类候选、分级候选、标签草稿、组织范围候选、质量评分、敏感提示 |
| 规则治理 | 增强结果 + active 治理规则集 | 正式分类、分级、标签、组织范围、索引准入、复核原因 |
| 版本判定 | 治理结果 + 质量报告 + 标准化引用 | `available` / `review_required` / `failed` |
| 索引准备 | 标准化对象 + 正式治理元数据 + 权限范围 | RAGFlow 同步包、metadata 投影 |

### 9.4 自动治理规则类型

| 治理项 | 规则输入 | 自动动作 | 人工介入条件 |
|--------|----------|----------|--------------|
| 数据域分类 | 标准化内容、来源类型、数据源默认提示、业务词典 | 写入 `business_domain` | 候选冲突或置信度低。 |
| 资产类型 | 文档结构、标题、来源路径、内容特征、schema | 写入 `asset_type` | 无法识别或与数据源配置冲突。 |
| 分级 | 数据域默认规则、敏感字段识别、字段类型、组织范围 | 写入 L1-L4 | L4、敏感冲突、跨组织风险或低置信度。 |
| 标签 | 标题、目录、正文摘要、结构化字段、业务词典 | 写入标签集合 | 置信度低或业务专家要求审核。 |
| 组织范围 | 数据源默认组织、提交人组织、内容组织实体、来源系统 | 写入 `org_scope` | 多组织冲突、无法映射、跨组织内容高风险。 |
| 质量准入 | 正文完整性、块结构、来源定位、字段完整性、切片可用性 | 允许或阻断进入可用状态 | 低于阈值。 |
| 复核触发 | 高敏字段、规则冲突、策略命中、管理员配置 | 进入 `review_required` | 命中即人工。 |
| 索引准入 | 分级、脱敏策略、切片质量、字段可索引性 | 允许、拒绝或要求复核 | L4 明文、权限范围不明、切片异常。 |
...
| 增强候选 | `domain_candidates`、`level_candidates`、`tag_candidates`、`org_candidates` | `metadata-enrich` |
| 质量信息 | `quality_score`、`missing_title`、`empty_content`、`field_completeness`、`chunk_readiness` | `quality_report` |
...
1. 质量准入优先，质量不达标时不继续生成可用版本。
2. 分类和资产类型先于分级，分级规则可依赖数据域和资产类型。
3. 标签规则可并行执行，但最终标签需去重、归一化和置信度过滤。
4. 组织范围规则在分类分级后执行，便于结合数据域、敏感级别和来源组织判断可见范围。
5. 复核触发规则后置执行，只要命中阻断条件，即使前面规则已给出结果，也进入 `review_required`。
6. 索引准入最后执行，避免 L4 明文字段、跨组织范围不明或切片异常资产进入 RAGFlow。
...
控制台一期至少提供规则列表、规则编辑、发布、启停、导入导出和决策追踪查看；复杂拖拽式规则编排可作为二期能力。
...
| `MetadataEnriched` | 治理候选生成完成 | `governance-rule` |
...
| 控制点 | 技术实现 |
|-------|---------|
| 分类分级 | 基于 `normalized_document` / `normalized_record` 由 `metadata-enrich` 生成候选，`governance-rule` 根据配置规则落正式结果。 |
| 标签治理 | 基于标准化对象内容生成草稿，通过标签规则归一化、去重和置信度过滤，低置信度或冲突时进入人工复核。 |
| 组织范围治理 | 结合数据源默认组织、提交人组织、内容实体和组织映射规则生成 `org_scope`，冲突时取更窄范围或人工复核。 |
| 生命周期 | `document_version` 状态机控制 `processing`、`available`、`review_required`、`archived`、`disabled`、`failed`。 |
| 版本回溯 | `raw_object`、`document_version`、`normalized_asset_ref`、`governance_result`、`index_manifest` 全链路可追溯。 |
| 质量复核 | `quality_report` + `review_required` 队列。 |
| 索引一致性 | `index_manifest` 记录索引分区、同步状态、投影版本和失败原因。 |
| 规则审计 | `governance_decision_log` 记录规则命中、冲突、置信度和人工覆盖。 |
...
审计日志需至少包含：操作主体、主体类型、操作时间、请求 ID、目标对象、动作类型、执行结果、来源 IP、脱敏动作、命中的权限策略、命中的治理规则和关联作业 ID。
...
以下能力在 v2.0 中仅做架构预留，不做一期具体设计与实现，也不作为一期交付验收项：
...
| 节点 | 角色 | 主要模块 |
|------|------|---------|
| 1 号节点 | 管控与元数据节点 | `nexus-api`、`nexus-console`、`identity-org-service`、`ingest-gateway`、`metadata-service`、`governance-rule`、`job-orchestrator`、`iam-audit-service`、PostgreSQL |
...
| 高可用升级 | v2.0 明确边界 | 拆分数据库、检索、对象存储和运行观测节点。 |
...
4. 运维能力在 v2.0 中仅保留基础工程接口，后续需单独输出运维设计文档。
...
| 规则可追溯 | 自动治理和人工覆盖均写入 `governance_decision_log`，可查看命中规则、置信度、冲突和最终结果。 |
| 版本状态简化 | 资产版本状态使用 `processing`、`available`、`review_required`、`archived`、`disabled`、`failed`，不存在强制全量人工审核。 |
| 自动优先人工兜底 | 规则命中、质量达标、无冲突时自动进入 `available`；只有异常、冲突、低置信度和高风险场景进入 `review_required`。 |
