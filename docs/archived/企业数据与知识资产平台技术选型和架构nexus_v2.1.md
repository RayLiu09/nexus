---
title: 企业数据与知识资产平台技术选型和架构nexus_v2.1
created: '2026-04-27'
modified: '2026-04-27'
---

# 企业数据与知识资产平台技术选型和架构 v2.1 — NEXUS
...
本文档基于 [企业数据与知识资产平台nexus_v7.0.md](/home/bjbodao/projects/nexus/docs/企业数据与知识资产平台nexus_v7.0.md)、[企业数据与知识资产平台需求Spec_v2.0.md](/home/bjbodao/projects/nexus/docs/企业数据与知识资产平台需求Spec_v2.0.md)、[企业数据与知识资产平台技术选型和架构nexus_v2.0.md](/home/bjbodao/projects/nexus/docs/企业数据与知识资产平台技术选型和架构nexus_v2.0.md) 和本轮架构 Review 意见，输出 NEXUS 一期工程落地的 v2.1 技术架构基线。
...
## 二、本轮 Review 结论与 v2.1 修正

### 2.1 Review 发现

| 序号 | Review 意见 | 既有问题 | v2.1 修正 |
|------|-------------|-----------|-----------|
...
| R-05 | 数据资产治理和质量评分需要引入 AI 大模型能力 | v2.0 的 `metadata-enrich` 偏规则候选生成，AI 主导能力、模型调用边界、提示词版本和证据链不足 | 新增 `llm-gateway` 与 `ai-governance-orchestrator`，由 AI 生成治理建议、质量维度评分、证据引用和置信度，规则引擎作为硬约束和准入闸门。 |
| R-06 | AI 主导不能演变为不可解释黑盒 | 仅保存最终治理结果会导致业务专家无法判断 AI 依据，也无法回灌优化 | 扩展 `quality_report`、`governance_decision_log` 和新增 `ai_governance_run`，记录模型配置、提示词版本、输入摘要、证据引用、候选值、评分维度、采纳状态和人工反馈。 |
...
10. AI 输出必须结构化、可校验、可追溯、可回放。任何进入正式治理结果或质量报告的 AI 结论，必须能关联模型配置、提示词版本、输入摘要、证据引用和采纳策略。
11. 大模型厂商和模型版本不得写死在业务流程中，统一通过 `llm-gateway` 和 `ai_model_profile` 适配，支持私有化模型、云端兼容接口和后续模型替换。
...
| `llm-gateway` | AI 大模型统一接入网关 | 模型配置、提示词版本、结构化输出校验、调用限流、调用审计、失败降级和供应商适配 | 资产主数据存储、最终治理状态切换、权限主策略判定 |
| `ai-governance-orchestrator` | AI 治理与质量评分编排能力 | 基于标准化对象生成分类、分级、标签、组织范围建议、质量评分、证据引用和置信度 | 绕过规则护栏直接发布资产、替代人工复核、直接读取未授权原始数据 |
...
9. AI 输出必须可解释。分类、分级、标签、组织范围和质量评分必须保留证据引用、置信度、模型配置、提示词版本和采纳状态。
10. 模型可替换。业务流程只依赖 `llm-gateway` 契约和结构化输出 Schema，不依赖具体厂商、模型名或提示词实现。
...
normalized_document / normalized_record
    │
    ├── llm-gateway（模型配置、提示词版本、结构化输出校验、调用审计）
    │
    ├── ai-governance-orchestrator（AI 治理建议、AI 质量评分、证据引用、置信度校准）
    │
...
说明：v2.1 不设置独立运维业务模块。发布、监控、告警、容量规划仅在接口、日志、健康检查和部署规范上预留扩展点。AI 大模型能力通过 `llm-gateway` 接入，不把具体模型实现写入资产主数据和治理规则。
...
| `llm-gateway` | 必做 | 统一封装大模型调用、模型配置、提示词版本、结构化输出校验、限流、审计和降级 | `ai_model_profile`、模型调用记录、结构化 AI 响应 |
| `ai-governance-orchestrator` | 必做，建议作为 `metadata-enrich` 内置子模块 | 基于标准化对象调度 AI 完成分类、分级、标签、组织范围建议、质量维度评分、证据抽取和置信度校准 | `ai_governance_run`、AI 治理建议、AI 质量评分明细 |
...
为避免在主表保存可推导字段，v2.1 引入只读视图或等价查询封装：
...
| `ai_model_profile` | `llm-gateway` | AI 模型与提示词配置 | 控制治理和质量评分使用的模型、提示词版本、输出 Schema 和脱敏策略。 |
| `ai_governance_run` | `ai-governance-orchestrator` | AI 治理与质量评分执行记录 | 基于标准化对象生成 AI 建议、质量评分、证据引用和置信度。 |
| `quality_report` | `metadata-enrich` / `ai-governance-orchestrator` | 质量报告 | 基于标准化对象和 AI 评分生成，规则准入可引用。 |
...
| 字段 | 类型 | 约束 | 说明 |
|------|------|------|------|
| `profile_id` | string | 主键，必填 | 模型配置 ID。 |
| `profile_name` | string | 必填 | 配置名称，如“数据资产治理评分默认配置”。 |
| `provider_type` | enum | 必填 | `openai_compatible` / `local_llm` / `private_cloud`。 |
| `model_name` | string | 必填 | 模型名称或部署名称。 |
...
2. 已发布配置不允许原地修改，模型、提示词、Schema 或脱敏策略变更必须创建新版本。
3. 外部云端模型调用默认使用 `metadata_only` 或 `masked_content`，涉及 L3/L4 明文时必须使用私有化模型或经过脱敏策略批准。
...
| `profile_id` | string | 必填，外键 | 使用的模型配置。 |
...
1. 同一 `version_id + profile_id + input_hash` 的 AI 执行应幂等，避免重复调用模型。
...
v2.1 延续 v2.0 简化状态模型，并将 AI 治理建议和 AI 质量评分纳入自动流转判断，减少人工审核负担。
...
9. `ai_model_profile` 中的模型配置、提示词版本、输出 Schema 和脱敏策略
...
| AI 治理与质量评分 | 标准化摘要 + 脱敏内容 + 模型配置 + 输出 Schema | AI 分类、分级、标签、组织范围建议、质量维度评分、证据引用、置信度 |
...
v2.1 将 AI 大模型作为数据资产治理和质量评分的主判断能力，但不允许模型绕过平台规则和权限。执行链路采用“三段式”：

1. AI 生成建议。`ai-governance-orchestrator` 基于 `normalized_document` / `normalized_record` 和脱敏上下文调用 `llm-gateway`，生成分类、分级、标签、组织范围、质量评分、证据引用和置信度。
...
1. `llm-gateway` 必须执行输入字段白名单、敏感脱敏、输出 Schema 校验、超时控制、重试控制和调用审计。
2. 外部模型默认不接收 L3/L4 明文内容；确需使用时必须采用脱敏上下文或私有化模型配置。
3. 模型响应不得直接进入 `governance_result`，必须经过规则护栏和状态机判定。
4. 提示词、模型配置、输出 Schema 和评分权重均应版本化，支持重跑 AI 治理和历史回溯。
...
| 模型上下文 | `profile_id`、`prompt_version`、`output_schema_version`、`redaction_policy` | `ai_model_profile` |
...
| `GET /ai/model-profiles` | 查询 AI 治理与质量评分模型配置。 |
| `POST /ai/model-profiles/{id}/validate` | 校验模型配置、提示词版本、输出 Schema 和脱敏策略。 |
...
控制台一期至少提供规则列表、规则编辑、发布、启停、导入导出、AI 评分查看、AI 建议采纳状态和决策追踪查看；复杂拖拽式规则编排、提示词在线调优和模型效果 A/B 可作为二期能力。
...
metadata-enrich 构造 AI 治理上下文
    ▼
llm-gateway / ai-governance-orchestrator 生成 AI 治理建议、质量评分和证据引用
    ▼
...
```text
规则升级 / 模型配置升级 / 解析失败 / 人工复核 / 索引失效 / AI 评分需校准
    ▼
...
| AI 模型网关 | `llm-gateway` | 统一模型配置、提示词版本、结构化输出 Schema 校验、脱敏策略、调用限流和审计。 |
...
| AI 治理与评分 | `version_id + profile_id + prompt_version + output_schema_version + input_hash` |
...
| `MetadataEnriched` | AI 输入上下文和敏感识别完成 | `ai-governance-orchestrator` |
...
| `AIModelProfilePublished` | AI 模型配置或提示词版本发布 | `llm-gateway`、`ai-governance-orchestrator` 缓存刷新 |
...
| AI 质量评分 | `ai-governance-orchestrator` 生成维度分、综合分、证据引用和修复建议，`quality_report` 保存有效评分，规则负责质量准入。 |
...
8. AI 模型配置发布、提示词版本变更、模型调用、AI 建议采纳、AI 质量评分人工校准。

审计日志需至少包含：操作主体、主体类型、操作时间、请求 ID、目标对象、动作类型、执行结果、来源 IP、脱敏动作、命中的权限策略、命中的治理规则、AI 模型配置版本、提示词版本和关联作业 ID。
...
以下能力在 v2.1 中仅做架构预留，不做一期具体设计与实现，也不作为一期交付验收项：
...
| 节点 | 角色 | 主要模块 |
|------|------|---------|
| 1 号节点 | 管控与元数据节点 | `nexus-api`、`nexus-console`、`identity-org-service`、`ingest-gateway`、`metadata-service`、`governance-rule`、`llm-gateway`、`job-orchestrator`、`iam-audit-service`、PostgreSQL |
| 2 号节点 | MinerU 解析、标准化与 AI 治理节点 | `parse-workers`、`normalize-service`、`metadata-enrich`、`ai-governance-orchestrator`、MinerU Router（可选）、本地大模型服务（可选） |
...
| 高可用升级 | v2.1 明确边界 | 拆分数据库、检索、对象存储、AI 推理和运行观测节点。 |
...
4. 运维能力在 v2.1 中仅保留基础工程接口，后续需单独输出运维设计文档。
...
| AI 模型网关可用 | `llm-gateway` 可按模型配置和提示词版本完成结构化调用，失败时有明确错误和降级状态。 |
| AI 质量评分可解释 | `quality_report` 必须包含维度分、综合分、证据引用、置信度、阻断原因和人工校准状态。 |
| AI 治理建议可追溯 | 分类、分级、标签、组织范围建议可回溯到 `ai_governance_run`、模型配置、提示词版本、输入摘要和证据引用。 |
