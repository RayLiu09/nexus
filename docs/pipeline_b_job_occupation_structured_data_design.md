...
| `Skill`                | `job_demand_requirement_item.item_type = professional_skill` / `tool` / `certificate`；`properties.item_type` 保留细分类，前端图例按职业技能 / 工具 / 证书拆分 |
| `ProfessionalLiteracy` | `job_demand_requirement_item.item_type = professional_literacy`                |
| `WorkTask`             | `occupational_work_task`                                                       |
| `WorkContent`          | `occupational_work_content`；岗位需求图谱中也可来自 `job_demand_requirement_item.item_type = work_task_candidate`，表示招聘记录抽取出的工作内容候选 |
...
| `JOB_RECORD_HAS_WORK_CONTENT`          | 岗位记录包含工作内容候选           |
| `JOB_ROLE_AGGREGATES_RECORD`           | 岗位角色聚合招聘记录               |
| `JOB_ROLE_REQUIRES_SKILL`              | 岗位角色要求技能，来自多条记录聚合 |
| `JOB_ROLE_REQUIRES_LITERACY`           | 岗位角色要求职业素养               |
| `JOB_ROLE_REQUIRES_WORK_CONTENT`       | 岗位角色要求工作内容，来自多条记录聚合 |
| `TASK_HAS_WORK_CONTENT`                | 任务包含工作内容                   |
| `TASK_REQUIRES_ABILITY`                | 任务要求能力条目；用于不要求关联工作内容的大类（如 PGSD G/S/D） |
...
| `job_demand_dataset`            | 知识块   | `列表` / `岗位图谱`。列表展示结构化记录、字段筛选、技能/素养/工作内容候选抽取结果、统计概览；岗位图谱只读消费 `CapabilityGraphStaging(build_type=job_demand)`，图例需拆分岗位角色、岗位记录、工作任务、职业技能、职业素养、工具、证书；呈现层按“岗位角色 -> 职业技能 / 职业素养 / 工作任务 / 工具与证书二级汇聚节点 -> 明细条目”组织，不回写 staging 表 |
| `occupational_ability_analysis` | 知识块   | `列表` / `能力图谱`。列表展示 PGSD 类别树、任务-工作内容-能力结构；能力图谱只读消费 `CapabilityGraphStaging(build_type=ability_analysis)`        |
