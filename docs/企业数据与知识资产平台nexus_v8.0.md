...
**backend 选择规则（MinerU v3.x）**

MinerU v3.x 将原 `model_version` 参数统一为 `backend`，支持以下模式：

| 文件类型 | backend | 说明 |
|---------|---------|------|
| HTML / XHTML | `pipeline` | HTML 原生解析，无需 GPU |
| PDF、Word、PPT、图片（默认） | `pipeline` | 成本低、速度快，适合大批量处理 |
| 复杂排版 / 图文混排 | `vlm-auto-engine` | 本地 VLM 推理，精度更高，需 GPU，仅支持中英文 |
| 远程 VLM 服务 | `vlm-http-client` | 对接 OpenAI 兼容服务器，适合无本地 GPU 场景 |
| 高精度多语言（下一代） | `hybrid-auto-engine` | 本地混合推理，精度最高，多语言支持 |

路由规则：在 `run_parse` 阶段由 `raw_object.mime_type` 自动选择，调用方可通过 Job payload 中的 `backend_override` 字段强制指定。

**normalize 阶段 body_markdown 生成策略对比**

MinerU 支持两种路径生成 `normalized_document.body_markdown`，各有适用场景：

| 对比维度 | Pipeline A：pipeline backend + mineru_converter | Pipeline A：vlm-auto-engine 直接 MD |
|---------|------------------------------------------------|-------------------------------------|
...
| **行间公式** | ✓ `$$\n{latex}\n$$` 标准格式 | ✓ `$$...$$` 标准格式，LaTeX 更简洁（无多余空格） |
| **行内公式** | ✓ 有效，LaTeX 含 MinerU 原始空格（如 `$p _ { t }$`） | ✓ 更简洁（如 `$p_t$`） |
| **表格** | ✓ Markdown `\|` 表格（由 `table_body.html` 转换） | HTML `<table>` 格式，需二次转换 |
| **图片引用** | 无图片链接（s3:// URI 存于 block 数据，不嵌入 markdown） | `![](images/xxx.jpg)` 相对路径，需配合 `return_images=true` |
| **图片内容理解** | ✓ 通过 LiteLLM VLM 提取图片/图表/表格内容，写入 block.content | ✗ 仅占位符，无内容描述 |
| **连字符断行** | ✓ `mineru_converter` 自动解决 PDF 物理行断行 | 少量残留（同一处） |
| **中文支持** | ✓ 正常 | ✓ 正常（仅支持中英文） |
| **处理速度** | 较快（CPU pipeline 模式） | 较慢（本地 VLM 推理） |
| **GPU 依赖** | 无 | 需要 GPU |
| **P0 默认策略** | ✓ **当前采用** | 备选，适合高精度场景 |

**当前 P0 实现选择 `pipeline backend + mineru_converter` 的原因：**
1. 无 GPU 依赖，适合 CPU Worker 大批量处理
2. 标题层级正确，表格直接输出 Markdown，结构更完整
3. 图片内容通过 LiteLLM VLM 独立分析，不受 MinerU backend 限制
4. `vlm-auto-engine` 可作为高精度场景的升级选项，通过 `backend_override` 按需启用

**OCR 自动开启规则**

MinerU v3.x 中 `pipeline` backend 的 OCR 由引擎内部自动判断，不再接受 `ocr_enable` 参数。以下为内部行为参考：

| 触发条件 | 说明 |
|---------|------|
| mime_type 包含 `image/` | 图片类文件自动开启 OCR |
| mime_type 为 `application/pdf` | PDF 自动开启 OCR（含扫描件场景） |
| mime_type 包含 `tiff` | TIFF 扫描件自动开启 OCR |
| 其他文档格式 | 依赖原生文本提取 |
