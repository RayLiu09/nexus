...
export function ResultTabs({ data }: ResultTabsProps) {
  const counts = useMemo(() => {
    const records = data.retrieval_results.reduce(
      (n, r) => n + (r.records?.length ?? 0),
      0,
    );
    const items = data.retrieval_results.reduce(
      ...
      <Tabs
        ...
        items={[
          ...
          },
          {
            ...
            children: (
              <ResultsList results={data.retrieval_results} kind="aggregations" />
            ),
          },
          {
            key: "source_refs",
...
function SourceRefsView({ refs }: { refs: RetrievalSourceRef[] }) {
  if (refs.length === 0) {
    return (
      <Empty
        image={Empty.PRESENTED_IMAGE_SIMPLE}
        description="当前执行没有产生 source_refs"
      />
    );
  }
  return <JsonPreview value={refs} maxHeight={480} label="source_refs" />;
}

function StepsView({ steps }: { steps: RetrievalConversationStep[] }) {
  if (steps.length === 0) {
    return (
      <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="无 conversation_steps" />
    );
  }
  return (
    <div className="flex flex-col gap-2">
      {steps.map((step, idx) => (
        <div key={`${step.step}-${idx}`} className="rounded-md border border-line p-3">
          <div className="mb-1 flex flex-wrap items-center gap-2">
            <Tag color="blue">{step.step}</Tag>
            <Tag color={step.status === "completed" ? "success" : "warning"}>
              {step.status}
            </Tag>
