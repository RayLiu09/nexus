...
import {
  buildClassificationDictionary,
  type ClassificationDictionaryEntry,
...
export default async function TagReviewPage() {
  const [result, rulesResult] = await Promise.all([
    getApiData<GovernanceReviewItem[]>("/internal/v1/governance-reviews/pending", [], {
      pageSize: "20",
    }),
    getApiData<{
      classifications?: ClassificationDictionaryEntry[];
      levels?: LevelDictionaryEntry[];
    }>("/internal/v1/admin/governance-rules", {}),
  ]);
  return (
    <>
      <PageHeader
        eyebrow="资产与治理"
        title="治理审核"
        description="业务专家在此确认数据分类、分级、结构化标签、质量处置与组织范围。"
        actions={
