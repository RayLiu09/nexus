...
export function JobDemandKnowledgeView({ normalizedRefId }: Props) {
  ...
  useEffect(() => {
    ...
    getApiData<DatasetListEnvelope>("/api/record-assets/job-demand-datasets", [], {
  ...
  if (!datasetState.dataset) {
    return (
      <Empty description="该 ref 没有关联的岗位需求数据集" image={Empty.PRESENTED_IMAGE_SIMPLE} />
    );
  }
...
function DatasetSummary({ dataset }: { dataset: JobDemandDataset }) {
  const validCount = Math.max(0, dataset.record_count - (dataset.invalid_count ?? 0));
  return (
    <Card title="数据集概要" size="small">
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Statistic
          ...
          styles={{ content: { color: "var(--warning-600)" } }}
        />
        <Statistic
          title="无效记录"
          value={dataset.invalid_count}
          styles={{ content: { color: "var(--danger-600)" } }}
        />
      </div>
  ...
  return (
    ...
    >
      ...
      <Table<JobDemandRecord>
        ...
        locale={{
          emptyText: <Empty description="无匹配记录" image={Empty.PRESENTED_IMAGE_SIMPLE} />,
        }}
      />
    </Card>
  );
  ...
  return (
    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Select
        placeholder="雇佣类型"
        allowClear
        value={filters.employment_type || undefined}
    ...
    <div className="flex flex-col gap-3">
      {record.job_description ? (
        <section>
          ...
          <Typography.Paragraph className="!mt-1 !mb-0 text-sm whitespace-pre-wrap">
            {record.job_description}
...
function ExtractedItemsDrawer({
  ...
  return (
    ...
    >
      ...
      {!state.loading && state.items.length > 0 ? <ItemsByType items={state.items} /> : null}
    </Drawer>
  );
}
  ...
  return (
    <Card title="质量问题" size="small">
      <ul className="m-0 list-none pl-0">
        {entries.map(([key, val]) => (
          <li key={key} className="py-1 text-sm">
