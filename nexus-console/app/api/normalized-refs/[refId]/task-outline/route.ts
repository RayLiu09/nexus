...
import type { TaskOutlineEnvelope } from "@/lib/api";
...
export async function GET(
  ...
  const result = await internalBackendGet<TaskOutlineEnvelope>(
    `/internal/v1/normalized-refs/${encodeURIComponent(refId)}/task-outline`,
  );
  if (!result.ok) {
