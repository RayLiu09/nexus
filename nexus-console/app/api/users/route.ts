 ...
 *
 * POST → create a console user (admin only — role enforced by middleware).
 * Server-side proxy so the client never touches /internal/v1 with the
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const result = await proxy<unknown>("/internal/v1/users", {
    method: "POST",
    body,
