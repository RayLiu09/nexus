...
def test_delete_unused_source_soft_deletes_and_audits(app, session):
    ...
    resp = client.delete(f"/v1/data-sources/{source.id}")
...
def test_delete_unused_source_is_idempotent(app, session):
    source = _seed_source(session)
    client = TestClient(app)
    first = client.delete(f"/v1/data-sources/{source.id}").json()["data"]
    second = client.delete(f"/v1/data-sources/{source.id}").json()["data"]
...
def test_delete_source_with_force_bypasses_dependency_check(app, session):
    source = _seed_source(session, with_raw=True)
    client = TestClient(app)
    resp = client.delete(f"/v1/data-sources/{source.id}?force=true")
    assert resp.status_code == 200
    session.refresh(source)
    assert source.deleted_at is not None
...
def test_list_data_sources_hides_deleted_by_default(app, session):
    ...
    client = TestClient(app)
    client.delete(f"/v1/data-sources/{dead.id}")

    resp = client.get("/v1/data-sources")
    ids = {row["id"] for row in resp.json()["data"]}
    assert live.id in ids
    assert dead.id not in ids

    resp_all = client.get("/v1/data-sources?include_deleted=true")
...
def test_delete_unknown_source_returns_404(app):
    client = TestClient(app)
    assert client.delete("/v1/data-sources/nope").status_code == 404
