...
def write(
    ...
    *,
    ...
    # We explicitly delete child rows first so the rerun works the same on
    # SQLite (FK CASCADE off by default) as on Postgres (FK CASCADE configured
    # in the migration). Caller commits later.
    ...
    if existing is not None:
        session.execute(
            delete(models.JobDemandRequirementItem).where(
                models.JobDemandRequirementItem.dataset_id == existing.id
            )
        )
        session.execute(
            delete(models.JobDemandRecord).where(
                models.JobDemandRecord.dataset_id == existing.id
            )
        )
