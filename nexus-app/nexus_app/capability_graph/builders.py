...
def build_course_standard(
    ...
    for row in rows:
        ...
        contents = row.get("course_contents")
        skills = row.get("skill_requirements")
        knowledge = row.get("knowledge_requirements")
        evidence = row.get("evidence") if isinstance(row.get("evidence"), dict) else {}
        if not module or not all(isinstance(value, list) and value for value in (contents, skills, knowledge)):
            continue
        ...
        for content_index, content in enumerate(contents, start=1):
            ...
            if not content_text:
                continue
            content_key = (NodeType.COURSE_CONTENT, f"{module_key[1]}:content:{row_index}:{content_index}:{content_text}")
            nodes.append(NodeSpec(*content_key, display_name=content_text, source_table="normalized_document", properties={"table_row_index": row_index, "source_block_ids": evidence.get("source_block_ids", [])}))
            edges.append(EdgeSpec(EdgeType.COURSE_MODULE_HAS_COURSE_CONTENT, module_key, content_key, source_table="normalized_document", evidence=evidence))
