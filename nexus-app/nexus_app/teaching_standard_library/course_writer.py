...
def write(
    ...
    desired_keys = {
        (course.course_type, course.standard_course_name)
    ...
    used_ids = {
        course.course_id for key, course in existing.items() if key in desired_keys
    }
    ...
    for fact in projection.courses:
        ...
        source_values = _source_values(fact, projection.extractor_version)
...
def source_fact_hashes(
    payload: dict[str, Any],
) -> dict[tuple[str, str], str] | None:
    """Return logical-course source hashes without mutating persisted rows."""
    projection = validate_projection(payload)
    if projection is None:
        return None
    return {
        (fact.course_type, fact.standard_course_name): _source_hash(
            _source_values(fact, projection.extractor_version)
        )
        for fact in projection.courses
    }


def _source_values(fact: Any, extractor_version: str | None) -> dict[str, Any]:
    return {
        ...
        "evidence_bindings": [
            item.model_dump(mode="json") for item in fact.evidence_bindings
        ],
...
def _allocate_course_id(
    ...
    identity = (
        library.major_code
        or f"S{hashlib.sha256(library.id.encode()).hexdigest()[:8].upper()}"
    )
