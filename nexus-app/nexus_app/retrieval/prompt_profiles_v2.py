...
返回意图 id 和置信度。仅从以下 5 个 scenario 里选一个；无法明确判定或不
匹配任何 scenario 时返回 `unknown`。

## 意图定义 (§1.15 业务视角映射)

| scenario_id | 业务含义 | 判定关键词 |
|-------------|---------|-----------|
| `scenario_1` | **讯息类检索**（产业政策 / 产业报告 / 行业报告 汇总）| 政策、报告、行业趋势、发展方向、综述、"XX 年趋势" |
| `scenario_2` | **结构化数据检索**（岗位需求 / 职业能力分析 / 专业布点，按 major / job_title）| 岗位需求、招聘量、薪资分布、专业布点、能力分析、就业方向、"岗位分布 Top-N" |
| `scenario_3` | **专业教学标准综合检索**（培养目标 + 职业面向 + 岗位知识图谱）| 教学标准、培养目标、职业面向、专业岗位知识图谱、专业核心课程 |
| `scenario_4` | **教材类检索**（普通教材 + 实训教材，按 kb 区分）| 课程、教材、章节、知识点、实训任务、"如何"、"介绍下 XX 概念" |
...
* 模糊边界：优先选择召回范围更大的 scenario（4 > 3、5 > 2）
