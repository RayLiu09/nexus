...
1. 先判断文档的用途和组织形态，再判断其主题领域；主题涉及某行业不等于该文档是行业报告。
2. 正文同时出现课程授课、教师/学生角色、知识点/学习目标、课堂互动、课后练习或作业等多个教学组织信号时，优先归类为 `course_textbook`（课程资源教材），即使教学主题是行业知识。
3. `sector_report`、`industry_report` 仅适用于具有报告性证据的材料，例如发布/研究主体、报告年份或统计周期、数据来源/统计口径、市场规模/样本数据、研究结论或趋势预测；不得仅因标题或正文出现“行业”而归类为报告。
4. 每个文档只能属于一个最具体的子类型；教学信号和报告信号冲突且无法排除时，降低置信度并说明冲突，不得臆造报告性证据。
5. `job_demand` 仅适用于实际岗位需求数据：正文必须出现可枚举的招聘记录，或至少同时具备岗位名称与另一项岗位字段（城市、企业、薪资、经验、学历、职责、任职要求、技能、发布时间等）。就业网站域名、页面标题、学校/企业简介、办学介绍、用人机制介绍或单独出现“就业”均不是岗位需求数据证据；缺少上述实质证据时不得归类为 `job_demand`。
6. 当正文包含专业代码，以及修业年限、培养目标、职业面向、主要课程、实践条件、证书、就业方向中的两项或以上时，优先归类为 `major_profile`。专业介绍中出现课程体系、师资或教学条件不构成 `course_textbook`；`course_textbook` 还必须具有面向学习者的章节教学组织证据，如学习目标、知识点、课堂互动、课后练习或作业。
7. 院校发布的专业、微专业或专业方向介绍，即使没有专业代码，只要正文同时具备培养/课程体系（或课程表）、学制/学分/学时、证书、招生对象、实践安排中的两项或以上，也优先归类为 `major_profile`。文中引用政策、行动计划或产业背景仅是办学背景，不得因此归类为 `industry_policy`；`industry_policy` 必须是该政策文件本身，具有发布主体、文号、条款或正式政策措施等证据。
8. 输出必须包含分类代码、置信度（0-1）和匹配依据，依据需引用文档中实际存在的结构或内容。
