...
class TestFieldMapping:
    ...
    def test_source_published_at_parsed_to_datetime(self, session):
        ...
        # The ISO8601 input "2024-12-12T01:12:59+08:00" parses to a tz-aware
        # datetime in the writer; we don't assert tzinfo here because SQLite
        # strips tz on roundtrip (test DB) while PostgreSQL (prod) keeps it.
        assert record.source_published_at.year == 2024
        assert record.source_published_at.month == 12
        assert record.source_published_at.day == 12
