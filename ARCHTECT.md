# NEXUS Architecture Contract v2.2

This document is a concise architecture baseline for implementation. It is distilled from `docs/企业数据与知识资产平台技术选型和架构nexus_v2.2.md`.
...
The P0 architecture optimizes for private deployment, local identity control, traceability, reduced manual review, and replaceable external engines.
...
3. Job and processing layer: `job-orchestrator`, RabbitMQ, Celery, parse workers, `normalize-service`.
...
5. Master data layer: `metadata-service` for assets, versions, governance results, quality reports, read models.
...
| `governance-rule` | Configurable rule sets, rule execution, conflicts, decision logs |
...
| `metadata-enrich` | Governance context, sensitivity recognition, quality report aggregation |
...
| `iam-audit-service` | RBAC, ABAC, masking, temporary auth, audit |

## Master Data Contract

Required P0 objects:

`org_unit`, `user_account`, `api_caller`, `data_source`, `ingest_batch`, `raw_object`, `document_asset`, `document_version`, `parse_artifact`, `normalized_asset_ref`, `normalized_document`, `normalized_record`, `ai_prompt_profile`, `ai_governance_run`, `quality_report`, `governance_rule_set`, `governance_rule`, `governance_result`, `governance_decision_log`, `knowledge_chunk`, `index_manifest`.
...
- `quality_report.version_id` is the relation from quality report to version; do not add reverse pointers.
...
- acceptable AI quality score or equivalent quality result;
...
- NEXUS owns `ai_prompt_profile`: model alias reference, task type, Prompt template/version, output schema, scoring weights, temperature, max input tokens, redaction policy, status, creator, publish time.
- `ai_prompt_profile` lifecycle: `draft` -> validate -> publish/active -> disable/archive. Published configs are immutable.
...
- `ai_governance_run` records version, normalized ref, Prompt profile, LiteLLM alias, input hash/summary, suggestions, quality scores, evidence refs, confidence, validation status, adoption status, human feedback.
...
- level override;
- tag suggestion and restriction;
- org scope inference;
- sensitive admission;
- quality admission;
...
1. Build governance context from normalized object, source hints, AI suggestions, quality report, sensitivity result, and org context.
2. Validate AI output schema and evidence.
3. Execute deterministic rules.
4. Resolve conflicts by priority, high-sensitivity-first policy, and configured strategy.
5. Write `governance_decision_log`.
6. Write or update `governance_result`.
7. Decide version status and index admission.

## Core Flows

Document ingestion:

`upload -> raw_object -> job -> MinerU parse -> normalized_document -> normalized_asset_ref -> AI governance -> quality_report -> rules -> governance_result -> available/review_required -> RAGFlow index`.

Structured/crawler ingestion:

`crawler/webhook/batch -> raw_object JSON -> normalized_record -> normalized_asset_ref -> AI governance -> quality_report -> rules -> governance_result -> index`.

Retrieval and QA:

`caller/user context -> auth verify -> search-service -> RAGFlow retrieval -> permission and masking -> rerank/context -> answer with source citations -> audit`.

Reprocess and re-governance:

Rule, Prompt, LiteLLM alias, parse failure, index failure, manual review, or scoring calibration may trigger reprocess, re-governance, AI re-score, or index rebuild. These flows must be idempotent and auditable.
...
| Cache | Redis 7.x |
| Async jobs | RabbitMQ, Celery, DLQ/retry |
...
- RBAC plus ABAC plus org scope plus data level control.
- All access must include caller and, where applicable, end-user context.
...
- Audit must cover access, approval, masking, version status change, reprocess, re-governance, Prompt config changes, LiteLLM alias changes, AI adoption, score calibration, rule publish, and API key changes.
...
- AI suggestions and quality scores are traceable to LiteLLM alias, Prompt profile, Prompt version, input summary, evidence, rule adoption, and human feedback.
