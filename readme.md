...
- Pipeline A `teaching_standard_library.v1` Slice-1 projections for normative professional teaching standards: one review-state standard row plus evidence-bound, source-scoped occupation and numeric-rule facts derived only from the persisted `normalized_document`. This model is independent from professional introductions (`major_profile.v1`), creates no global occupation master data, and treats public/professional/practice/elective/internship hour constraints as potentially overlapping rather than additive.
