...
- Professional teaching-standard documents may produce an independent
  ...
  added together. The parent does not persist an unused standard identifier;
  ...
  not escaped JSON text.
  Core-course tags use course-owned evidence. Foundation and extension course
  tags may also use the standard's evidence-bound training specification when
  course-owned text is insufficient; tool evidence remains course-owned.
  Activation, APIs, retrieval, and Console views remain later slices.
- Historical professional teaching-standard processing is exposed only as a
  dry-run-first internal command. It scans generated normalized documents whose
  latest official governance classification is `teaching_standard`, and skips
  documents with no structurally extracted course facts,
  reports bounded candidate/call/failure counts without writes or model
  traffic by default, and requires explicit apply mode to reuse the existing
  standard/course projection and derivation path. Applied rows remain in
  `review`; the command cannot activate or supersede a standard.
...
- Standard-document capability projection supports two distinct, normalized-table
  ...
  project `Course -> CourseModule -> CourseContent`, with parallel
  `SkillRequirement` and `KnowledgeRequirement` children under each course content.
...
- **专业简介业务视图**: `/asset-center/major/profiles` uses a server-paginated
  `major_profile` list. Its outer columns are professional name, professional
  code, study duration, education level, and institution name. Expanding a row
  lazily loads a non-table information panel containing occupation orientation,
  training positioning, ability requirements, courses and practical training,
  and certificates; unopened rows do not load child collections. One
  normalized document may contribute multiple professional rows. The list
  admits only the current catalog-visible version/ref whose latest official
  classification is `major_profile` or historical `program_profile`.
  Technical Asset Detail retains generic RAG/document chunks and no longer
  presents the professional-profile directory or graph view.
...
- **标准课程库业务视图**: the expandable outer table joins parent professional
  ...
  requirement, and four inferred tag groups. `血缘追溯` retains basis, keyword,
